    ...
    def __init__(self):
        ...
        for i in range(10):
            self.sMenu.addAction( QAction('Item %s' % i, self,
                                          triggered = lambda checked, x=i : self.action(x)))

        # The triggered signal has the checked arguments, this means that the "item" argument 
        # you use will be "overwritten" by the triggered checked status.
